...
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql+psycopg2://postgres:VasmiGroup#123@localhost:5432/js_insurance'
...
#Showing interests for admin
@app.route("/interests")
def interests():
    motor_interests = MotorInsurance.query.all()
    health_interests = HealthInsurance.query.all()
    travel_interests = TravelInsurance.query.all()
    workmen_interests = WorkmenInsurance.query.all()
    property_interests = PropertyInsurance.query.all()

    return render_template('interests.html', motor_interests=motor_interests)
